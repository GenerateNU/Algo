...
export const getAllUsers = async (): Promise<User[]>  => {
    ...
    const response: AxiosResponse<User[]> = await axios.get<User[]>(`http://${API_LINK}/users`);
    console.log(response.data)
    return response.data;
}

export const getCallbackUrl = async (id: number): Promise<Redirect> => {
    const response: AxiosResponse<Redirect> = await axios.get<Redirect>(`http://${API_LINK}/users/etrade-redirect/${id}`)
    console.log(response.data);
    return response.data;
}
